namespace NFiveHtb.Server.Controllers
{
    ...
    public class SessionController : ConfigurableController<SessionConfiguration>
	{
        ...
        public override string Name => "NFive/Session";
		...
		private async void OnConnecting(ICommunicationMessage e, IClient client, ConnectionDeferrals deferrals)
		{
			...
			using (var transaction = context.Database.BeginTransaction())
			{
				...
				// steveski - Need to work out if this is worth finding a way to do it in EF Core
		...
		{
			if (clientVersion != typeof(ServerMain).Assembly.GetName().Version.ToString())
			{
				this.Logger.Warn($"Client version does not match server version, got {clientVersion}, expecting {typeof(ServerMain).Assembly.GetName().Version}, dropping client: {e.Client.Handle}");
